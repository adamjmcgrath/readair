...
$y=$m=$d=null;return w;};$P.getISOWeek=function(){$y=this.getUTCFullYear();$m=this.getUTCMonth()+1;$d=this.getUTCDate();return this.getWeek();};$P.setWeek=function(n){return this.moveToDayOfWeek(1).addWeeks(n-this.getWeek());};$D._validate=function(n,min,max,name){if(typeof n=="undefined"){return false;}else if(typeof n!="number"){throw new TypeError(n+" is not a Number.");}else if(n<min||n>max){throw new RangeError(n+" is not a valid value for "+name+".");}
...
function date_format__php_to_js ( format )
{	/**
	 * Convert a php date format to js
	 * 
	...
	var result = format.replace(/(%|\\)?[a-zA-Z]/g, function(match, index, line){
		// Prep
		var s = match.substring(1);
		if ( s === '\\' ) return match; // skip, is literal
		var result = '';
		
		// Switch
		switch (match)
		{
			// seconds, leading zeroes
			case 's':
			case 'ss':
			case '%S':
				result = 'ss';
			...
			// minutes, leading zeroes
			case '%M':
			case 'i':
				result = 'mm';
				break;
				
			// 12-hour, without leading zeroes
			case 'g':
				result = 'h';
				break;
			// 12-hour, with leading zeroes
			case 'h':
			case '%I':
				result = 'hh';
				break;
				
			// 24-hour, without leading zeroes
			case 'G':
				result = 'H';
				break;
			// 24-hour, with leading zeroes
			case 'H':
			case '%H':
				result = 'HH';
				break;
			...
			case '%d':
				result = 'dd';
				break;
			// day of month, abbreviated
			case 'D':
			case '%a':
				result = 'ddd';
				break;
			// day of month, full name
			case 'l':
			case '%A':
				result = 'dddd';
				break;
			...
			// shortcut
			case '%D':
				result = date_format__php_to_js('%m/%d/%y');
			case '%r':
				result = date_format__php_to_js('h:i:s %p');
				break;
			case '%R':
				result = date_format__php_to_js('H:i');
				break;
			case '%T':
				result = date_format__php_to_js('%H:%M:%S');
				break;
...
{	// We are converting a PHP date format to a Datejs format
	format = date_format__php_to_js(format);
